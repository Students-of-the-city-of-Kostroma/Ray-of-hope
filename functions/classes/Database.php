...
class Database
{
    ...
    public function getPosts($count, $start_id = null)
    {
        ...
        $posts = array();
        while ($row = mysqli_fetch_assoc($result)) {
            $posts[] = array_filter($row, 'strlen');
        }
        foreach ($posts as &$post) {
            $images = ($this->mysqli->query("SELECT image.url AS post_image FROM image WHERE image.note=" . $post['id']));
            $imagesList = array();
    ...
    {
        $this->connect();
        $query = 'INSERT INTO note (author, publication_date, description, type_note) VALUES (' . $_SESSION['logged_org'] . ', NOW(), \'' . $_POST['text'] . '\', ' . $_POST['typepost'] . ')';
        $result = ($this->mysqli->query($query) or die("Ошибка sql запроса: " . $this->mysqli->error . " Запрос: " . $query));
        $note_id = ($this->mysqli->insert_id);
        ...
        foreach ($done_files as $url_to_img) {
